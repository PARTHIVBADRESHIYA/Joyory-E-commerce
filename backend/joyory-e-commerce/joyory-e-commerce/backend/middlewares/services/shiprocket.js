...
// ✅ Create Order + Assign Courier (with full debug)
export async function createShiprocketOrder(order) {
    const token = await getShiprocketToken();
    ...
    const shipmentData = {
        order_id: order._id.toString(),
        order_date: new Date(order.createdAt).toISOString(),
        pickup_location: "Primary", // 🔴 must match nickname in dashboard
        billing_customer_name: order.customerName || order.user?.name || "Guest",
    ...
    try {
        awbRes = await axios.post(
            "https://apiv2.shiprocket.in/v1/external/courier/assign/awb",
            { shipment_id: shipmentId, courier_id: "" }, // auto-assign
            { headers: { Authorization: `Bearer ${token}` } }
        );
        console.log("📦 Shiprocket AWB Response:", awbRes.data);
    } catch (err) {
    ...
    // ✅ Step 3: Build shipment details safely
    const shipmentDetails = {
        shiprocket_order_id: shiprocketOrderId,
        shipment_id: shipmentId,
        awb_code: awbRes.data?.response?.awb_code || null,
    ...
    // ✅ Always return raw API responses so you know the real reason for null
    return {
        shipmentDetails,
        rawResponses: {
            orderRes: orderRes.data,
