...
export const razorpayWebhook = async (req, res) => {
    try {
        ...
        // ✅ Handle raw body (Razorpay requires unmodified payload)
        const rawBody =
        ...
        try {
            ...
            console.error("⚠️ Failed to parse Razorpay webhook body:", err);
            return res.status(200).json({ status: "ignored", reason: "invalid JSON" });
        }

        // ✅ Guard for missing or invalid event
        const event = eventPayload?.event;
        if (!event || typeof event !== "string") {
            console.warn("⚠️ Missing or invalid Razorpay event field:", eventPayload);
        ...
        // ✅ Always respond fast to Razorpay
        return res.status(200).json({ status: "ok" });
    } catch (err) {
        console.error("🔥 Razorpay Webhook Error:", err);
        return res.status(200).json({ status: "error_logged" });
