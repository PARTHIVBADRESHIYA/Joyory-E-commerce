...
export const addCategory = async (req, res) => {
    try {
        ...
        // ✅ Fetch newly created category with full details
...
export const updateCategory = async (req, res) => {
    try {
        ...
        if (req.files?.bannerImage && req.files.bannerImage[0]) category.bannerImage = req.files.bannerImage[0].path;
        if (req.files?.thumbnailImage && req.files.thumbnailImage[0]) category.thumbnailImage = req.files.thumbnailImage[0].path;
