...
// /**
//  * 🔹 Helper to build a safe brand filter
//  * Supports ObjectId ref, plain string name, or stringified ObjectId
//  */
// const buildBrandFilter = (brand, extraFilter = {}) => {
//     return {
//         ...extraFilter,
//         $or: [
//             { brand: brand._id },                   // ObjectId
//             { brand: brand.name },                  // plain string
//             { brand: brand._id.toString() }         // stringified ObjectId
//         ]
//     };
// };
...
//                         { brand: { $in: brands.map(b => b.name) } },
//                         { brand: { $in: brands.map(b => b._id.toString()) } }
//                     ]
//                 }
//             },
...
//             ...b,
//             count: countMap[b._id.toString()] || countMap[b.name] || 0
//         }));
...
//  * Returns brand info + all products for that brand
//  */
...
//         // Find the brand document first
//         const brand = await Brand.findOne({
//             $or: [{ slug: brandSlug }, { name: brandSlug }]
//         }).lean();
...
//         // CORRECTED: Query products ONLY using the brand's ObjectId.
//         // The migration script has ensured all products are now linked this way.
...
//         // Find brand
//         let brand;
//         if (mongoose.Types.ObjectId.isValid(brandSlug)) {
//             brand = await Brand.findById(brandSlug).lean();
//         }
//         if (!brand) {
//             brand = await Brand.findOne({
//                 $or: [
//                     { slug: brandSlug },
//                     { name: slugToRegex(brandSlug) }
//                 ],
//                 isActive: true
//             }).lean();
//         }
...
//         // CORRECTED: Create the filter using the ObjectIds, as the database is now consistent.
//         const filter = { 
//             brand: brand._id,
