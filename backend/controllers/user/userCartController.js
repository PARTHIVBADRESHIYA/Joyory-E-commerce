...
export const getCartSummary = async (req, res) => {
  try {
    ...
    /* -------------------- 🛒 Final Cart Items (Use exact displayPrice logic) -------------------- */
    const round2 = (n) => Math.round(n * 100) / 100;
    const now = new Date();
    ...
    const finalCart = validCartItems.map((item) => {
      ...
      // ✅ Use your existing helper to calculate final displayPrice
      const enrichedVariants = calculateVariantPrices(
        variant ? [variant] : productDoc.variants || [],
        productDoc,
        activePromotions
      );

      const enrichedVariant = enrichedVariants[0];
      ...
      return {
        ...
        variant: {
          ...
          image: enrichedVariant?.images?.[0] || productDoc.images?.[0],
    ...
    const bagMrp = round2(
      finalCart.reduce(
        (sum, item) => sum + (item.variant.originalPrice || 0) * item.quantity,
        0
      )
    );
    ...
    const bagPayable = round2(
      finalCart.reduce(
        (sum, item) => sum + (item.variant.displayPrice || 0) * item.quantity,
        0
      )
    );
