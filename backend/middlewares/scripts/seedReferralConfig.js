...
// scripts/seedReferralConfig.js
import mongoose from "mongoose";
import dotenv from "dotenv";
import ReferralConfig from "../../models/ReferralConfig.js"; // adjust relative path
...
async function seedReferralConfig() {
    try {
        ...
        const defaultConfig = {
            ...
            minOrderAmount: 999,
            tiers: [
                { milestone: 1, reward: "Free Shipping Voucher" },
                { milestone: 3, reward: "Free Mini Lipsticks" },
                { milestone: 5, reward: "Early Access to Sales" },
            ],
        };
        ...
        console.log("🎉 ReferralConfig seeded/updated successfully:");
        console.log(config);

        process.exit(0);
    } catch (err) {
        console.error("❌ Seeding ReferralConfig failed:", err);
        process.exit(1);
    }
}
