...
//the above aprt for retunr and that are ok,. tilll 16/01/26 ,.. bottom code is without that refund queue ok,. 
...
export const scanPendingRefunds = async () => {
    const orders = await Order.find({
        ...
        "shipments.returns.refund.status": { $in: ["pending", "locked", "retrying"] },
        paid: true
    });

    for (const order of orders) {
        for (const shipment of order.shipments) {
            if (!shipment.returns) continue;

            for (const ret of shipment.returns) {

                if (ret.status !== "qc_passed") continue;

                if (!ret.refund) continue;

                if (!["pending", "locked", "retrying"].includes(ret.refund.status)) {
                    continue;
                }

                if (
                    ret.refund.status === "retrying" &&
                    ret.refund.nextRetryAt &&
                    ret.refund.nextRetryAt > new Date()
                ) {
                    continue; // ⛔ wait for retry window
                }
                ...
                console.log(
                    `💸 Enqueuing refund | Order ${order._id} | Return ${ret._id}`
                );

                if (ret.refund.status === "pending") {
                    const amount = calculateReturnRefundAmount(order, shipment, ret);

                    if (!amount || amount <= 0) continue;

                    ret.refund.amount = amount;
                    ret.refund.status = "locked";
                    ret.refund.lockedAt = new Date();
                    ret.refund.attempts = 0;
                    ret.refund.failureReason = null;

                    // ✅ ADD THIS
                    ret.refund.idempotencyKey = `refund_${order._id}_${ret._id}`;

                    await order.save();
                }


                if (ret.refund.gatewayRefundId) {
                    continue; // ⛔ refund already initiated
                }

                if (["locked", "retrying"].includes(ret.refund.status)) {
                    await addRefundJob(order._id, ret._id);
                }
...
//     const orders = await Order.find({
//         "shipments.returns.status": "qc_passed",
//         "shipments.returns.refund.status": { $in: ["pending", "locked"] },
//         paid: true
//     });
...
//                 // 🔒 HARD CONDITIONS (based on your document)
//                 if (
//                     ret.status !== "qc_passed" ||
//                     !["pending", "locked"].includes(ret.refund?.status)
//                 ) continue;
...
//                     if (!amount || amount <= 0) {
//                         console.error(`❌ Invalid refund amount for return ${ret._id}`);
//                         continue;
//                     }
