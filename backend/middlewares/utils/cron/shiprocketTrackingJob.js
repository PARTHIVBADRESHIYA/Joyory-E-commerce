...
function mapTimelineToNykaa(events = []) {
    ...
    for (const ev of events) {
        ...
        if (s.includes("pickup") || s.includes("manifest") || s.includes("ship")) {
...
function sanitizeTimelineEvent(event) {
    ...
    // Try 2: DD-MM-YYYY HH:mm
    if (!parsedDate || isNaN(parsedDate.getTime())) {
        const dm = /^(\d{2})-(\d{2})-(\d{4}) (\d{2}):(\d{2})$/;
        const match = event.date?.match(dm);
        if (match) {
            parsedDate = new Date(
                `${match[3]}-${match[2]}-${match[1]}T${match[4]}:${match[5]}:00`
            );
        }
    }

    // FINAL fallback → use shipment create time (NOT now)
    if (!parsedDate || isNaN(parsedDate.getTime())) {
        parsedDate = new Date(
            Date.now() - 1000 * 60 * 30
        ); // 30 min old to avoid jumping to top
    }
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (trackList.length > 0) {
                                ...
                                const formattedEvents = trackList
